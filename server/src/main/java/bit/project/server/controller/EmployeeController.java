...
public class EmployeeController{
    ...
    public Page<Employee> getAllBasic(PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_APPOINTMENT, UsecaseList.UPDATE_APPOINTMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_BRANCHROLEASSIGNMENT, UsecaseList.UPDATE_BRANCHROLEASSIGNMENT, UsecaseList.ADD_PAYROLL, UsecaseList.UPDATE_PAYROLL);
    ...
    public ResourceLink add(@RequestBody Employee employee, HttpServletRequest request) throws InterruptedException {
        ...
        for(Educationqualification educationqualification : employee.getEducationqualificationList()) educationqualification.setEmployee(employee);
        for(Workexperience workexperience : employee.getWorkexperienceList()) workexperience.setEmployee(employee);
    ...
    @GetMapping("bybranchanddepartment/{bid}/{did}")
    public List<Employee> getAllByBranchAndDepartment(@PathVariable Integer bid,@PathVariable Integer did,PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_APPOINTMENT, UsecaseList.UPDATE_APPOINTMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_BRANCHROLEASSIGNMENT, UsecaseList.UPDATE_BRANCHROLEASSIGNMENT, UsecaseList.ADD_PAYROLL, UsecaseList.UPDATE_PAYROLL);
         List<Employee> employees = employeeDao.findAll();
         ArrayList<Employee> employeeArrayList = new ArrayList<>();
         employees.forEach(employee -> {
             employee.getBranchList().forEach(branch -> {
                 employee.getEmployeeAppointmentList().forEach(appointment -> {
                     if (branch.getId().equals(bid) && appointment.getDesignation().getDepartment().getId().equals(did) && employee.getEmployeestatus().getId().equals(1)){
    ...
    public List<Employee> getAllByBranch(@PathVariable Integer id,PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_APPOINTMENT, UsecaseList.UPDATE_APPOINTMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_BRANCHROLEASSIGNMENT, UsecaseList.UPDATE_BRANCHROLEASSIGNMENT, UsecaseList.ADD_PAYROLL, UsecaseList.UPDATE_PAYROLL);
        List<Employee> employees = employeeDao.findAll();
        ArrayList<Employee> employeeArrayList = new ArrayList<>();
        employees.forEach(employee -> {
            employee.getBranchList().forEach(branch -> {
                if (branch.getId().equals(id) && employee.getEmployeestatus().getId().equals(1)){
                    employeeArrayList.add(employee);
                }
            });
        });
    ...
    @GetMapping("forapointment/{id}")
    public List<Employee> getAllForApointment(@PathVariable Integer id,PageQuery pageQuery, HttpServletRequest request){
        accessControlManager.authorize(request, "No privilege to get all employees' basic data", UsecaseList.SHOW_ALL_EMPLOYEES, UsecaseList.ADD_APPOINTMENT, UsecaseList.UPDATE_APPOINTMENT, UsecaseList.ADD_ATTENDANCE, UsecaseList.UPDATE_ATTENDANCE, UsecaseList.ADD_BRANCHROLEASSIGNMENT, UsecaseList.UPDATE_BRANCHROLEASSIGNMENT, UsecaseList.ADD_PAYROLL, UsecaseList.UPDATE_PAYROLL);

        List<Employee> employees = employeeDao.findAll();
        ArrayList<Employee> employeeArrayList = new ArrayList<>();
        employees.forEach(employee -> {
           if(employee.getEmployeeAppointmentList() != null){
               employee.getEmployeeAppointmentList().forEach(appointment -> {
                   if (appointment.getAppointmentstatus().getId().equals(1)){
                       employeeArrayList.add(employee);
                   }
               });
           }
        });
        employees.removeAll(employeeArrayList);
        return employees;
    }

    @GetMapping("/forpayroll")
    public List<Employee> getAllForPayroll(PageQuery pageQuery, HttpServletRequest request){
        ...
        ArrayList<Employee> employeeArrayList = new ArrayList<>();
        employees.forEach(employee -> {
            if(employee.getEmployeePayrollList() != null){
                employee.getEmployeePayrollList().forEach(payroll -> {
                    if (payroll.getPaydate().getMonthValue() == localDate.getMonthValue()){
                        employeeArrayList.add(employee);
                    }
                });
            }
