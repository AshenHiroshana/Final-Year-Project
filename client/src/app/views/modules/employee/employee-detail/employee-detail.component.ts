...
export class EmployeeDetailComponent extends AbstractComponent implements OnInit {
  ...
  async pdfViwer(id: string): Promise<void> {
      const fileObject = await this.fileService.getFile(id);
      this.urlSafe = this.sanitizer.bypassSecurityTrustResourceUrl( fileObject.file);
  }
}
