...
@Component({
  selector: 'app-consumeitem-form',
  templateUrl: './consumeitem-form.component.html',
  styleUrls: ['./consumeitem-form.component.scss']
})
export class ConsumeitemFormComponent extends AbstractComponent implements OnInit {
...
async loadData(): Promise<any>{
  ...
  async submit(): Promise<void> {
    ...
    consumeitem.consumeitemcategory = this.consumeitemcategoryField.value;
    consumeitem.name = this.nameField.value;
    consumeitem.unit = this.unitField.value;
    consumeitem.qty = this.qtyField.value;
    const photoIds = this.photoField.value;
    if (photoIds !== null && photoIds !== []){
      consumeitem.photo = photoIds[0];
    }else{
      consumeitem.photo = null;
    }
    consumeitem.vendorList = this.vendorsField.value;
    consumeitem.description = this.descriptionField.value;
    try{
      const resourceLink: ResourceLink = await this.consumeitemService.add(consumeitem);
