...
export class EmployeeService {
  ...
  async getAllByBranchAndDepartment(bid: number, did: number): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/bybranchanddepartment/${bid}/${did}`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }

  async getAllByBranch(id: number): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/bybranch/${id}`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }

  async getAllForApointment(id: number): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/forapointment/${id}`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }

  async getAllForPayroll(): Promise<Employee[]>{
    let employeeDataPage = await this.http.get<Employee[]>(ApiManager.getURL(`employees/forpayroll`)).toPromise();
    employeeDataPage = employeeDataPage.map((employee) => Object.assign(new Employee(), employee));
    return employeeDataPage;
  }
